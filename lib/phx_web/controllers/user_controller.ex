defmodule PhxWeb.UserController do
  ...
  def create(conn, params) do
    ...
    case status do
      :created ->
        conn |> put_status(:created) |> render("user-created.json", user: content)
  ...
  def update(conn, params) do
    ...
    case status do
      ...
      :no_result ->
        conn |> put_status(:not_found) |> render("user-not-found.json", user_id: user_id)
  ...
  def delete_user(conn, params) do
    ...
    case res do
      {:deleted, _schema} ->
        conn |> put_status(:ok) |> render("user-deleted.json", user_id: user_id)

      {:not_found, %Ecto.NoResultsError{}} ->
        conn |> put_status(:not_found) |> render("user-not-found.json", user_id: user_id)
  ...
  def get_points(conn, params) do
    ...
    case points do
      ...
      _ ->
        conn |> put_status(:ok) |> render("user-points.json", user_id: user_id, points: points)
    end
  end
  ...
  swagger_path :auth do
    ...
    description(
      "Authentication user with identifier and password. Identifier can be document or email"
    )
  ...
  def auth(conn, %{"identifier" => identifier, "password" => password}) do
    case SessionService.new(%{"identifier" => identifier, "password" => password}) do
      {:ok, token, user} ->
        conn |> put_status(:ok) |> render("authenticated.json", token: token, user: user)
